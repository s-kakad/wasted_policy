...
#import applicable food waste file from script 2
applicable_fw_tons_df <- read.csv("applicable_fw_tons.csv")

#import master file for likely food waste quantification
total_fw_tons_df <- read.csv("fw_masterfile_2022.csv") # suffix as per above

#rename column names to remove year stamp (data still in US short tons)
total_fw_tons_df <- total_fw_tons_df %>% rename(total_fw = total_fw_2022,
...
#run for loop to add a new row for solution_levels "All" and "Updated" to find total per state diversion
#since they are a combination of different solutions, we need to run a for loop for "All" and "Updated" scenarios separately
#first for the "All" (original EPA methodology)
for (s in state.abb) {
...
#run a second loop for the "Updated" scenario (reflecting the EPA's updated methodology)
for (s in state.abb) {
