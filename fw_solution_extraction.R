...
#the following files only contain selected solutions in the context of this policy analysis
#due to the waterfall methodology, two datasets are needed, one for each scenario
#this is because adding or removing a solution will influence the value for all remaining solutions
...
solutions_baseline_df <- solutions_baseline_df %>% select(state,
                                              solution_group, 
                                              solution_name,
                                              annual_tons_diversion_potential) # US short tons
...
#create vectors with relevant solutions ("Updated" represents the updated EPA methodology and = All - Recycling)
...
write.csv(applicable_fw_tons_df, "applicable_fw_tons.csv", row.names = FALSE)
